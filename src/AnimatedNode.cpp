...
Animator::Animation* AnimatedNode::findAnimation(const string& name)
{
	return mAnimator.findAnimation(name);
}

void AnimatedNode::addFrames(const string& name, sf::Vector2i startFrom, const sf::Vector2i& frameSize, size_t frames)
{
	Animator::Animation* animation = mAnimator.findAnimation(name);
	animation->AddFrames(startFrom, frameSize, frames);
}
	
void AnimatedNode::addFrames(const string& name, vector<sf::IntRect> frames)
{
	Animator::Animation* animation = mAnimator.findAnimation(name);
