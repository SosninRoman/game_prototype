...
bool Level::loadFromFile(const string& filename, TextureHolder& textures)
{
	...
	while(tileset != nullptr)
	{
		...
		TileSheet& map_sheet = textures.get(source);
		int first_map_id = tileset->IntAttribute("firstgid");

		mSheets.insert(std::pair<int,const TileSheet&>(first_map_id,map_sheet));
	...
	{			
		...
		while(tile != nullptr)
		{
			...
			if( tileid >= mFirstTileId)
			{
				...
				const TileSheet& map_sheet = itr->second;
				//
...
sf::IntRect Level::getFrame(int tileid, const TileSheet& map_sheet)
{
...
LevelObject* Level::getObject(const string& name)
{
...
vector<LevelObject>& Level::getAllObjects()
{
	return mObjects;
}